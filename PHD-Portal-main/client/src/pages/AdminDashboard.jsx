...
import {
  ...
  TrendingUp, CheckSquare, Loader2,
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const [stats, setStats]               = useState({ total: 0, eligible: 0, newApplications: 0, nbhm: 0 })
  ...
  const fetchApplications = useCallback(async (f = filters) => {
    ...
    try {
      ...
      const eligible = data.filter(a => a.eligibility_status === 'Eligible').length
      const newApplications = data.filter(a => a.is_new_application).length
      setStats({ total: data.length, eligible, newApplications, nbhm })
    } catch {
    ...
    <div className="min-h-screen bg-hero-gradient">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="animate-slide-up">
          ...
          {!loading && stats.newApplications > 0 && (
            <div className="mt-4 rounded-xl border border-accent-500/25 bg-accent-500/10 px-4 py-3 text-sm text-accent-200">
              New Application: {stats.newApplications} recent submission{stats.newApplications !== 1 ? 's' : ''} waiting in the dashboard.
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-slide-up">
          ...
          <div className="stat-card">
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className="text-white/50 text-sm">Eligible Applicants</span>
            </div>
            <p className="text-3xl font-bold font-heading text-white">
              {loading ? '—' : stats.eligible}
            </p>
          </div>
          ...
          <div className="stat-card">
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className="text-white/50 text-sm">New Applications</span>
            </div>
            <p className="text-3xl font-bold font-heading text-white">
